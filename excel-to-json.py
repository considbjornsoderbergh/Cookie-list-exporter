...
# First, collect unique categories and their descriptions
...
# Now group by category and domain
for category in df['Cookie category'].unique():
...
# Create output filename based on Excel file name
...
print(f"Total categories: {len(notice_table)}")
for category in notice_table:
    print(f"  - {category['cookie_category']}: {len(category['cookie_list'])} domains")
